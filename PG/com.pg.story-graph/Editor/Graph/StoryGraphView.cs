...
public sealed partial class StoryGraphView : GraphView
{
    ...
    private string CutCopyOperation(IEnumerable<GraphElement> elements)
    {
        ...
        foreach (var element in elements)
        {
            ...
            {
                StoryNode scenarioNode = ScriptableObject.Instantiate(nodeView.storyNode);
                scenarioNode.guid = GUID.Generate().ToString();
                scenarioNode.nodePosition += _createdNodeOffset * _pasteIndex;
                _clipboardNodes.copiedNodes.Add(scenarioNode);
            }
        }
    ...
    private void InitializeObjects()
    {
        if (window != null)
        {
            ...
            centerButton.clicked += () => viewTransform.position = storyGraph.rootNode.nodePosition;
        }
    }
    ...
    void BackToLayer(StoryGraph storyGraph)
    {
    ...
    public override List<Port> GetCompatiblePorts(Port startPort, NodeAdapter nodeAdapter)
    {
        ...
        {
            // Порты с именем "variable" могут подключаться только к другим портам с именем "variable"
            if (startPort.portColor == Color.yellow)
            {
                return endPort.portColor == Color.yellow &&
                       endPort.direction != startPort.direction && // Проверяем направление
                       endPort.node != startPort.node; // Проверяем, что это не один и тот же узел
            }

            // Все остальные порты могут подключаться к любым другим портам, кроме тех, у которых имя "variable"
            return endPort.portName != "variable" &&
                   endPort.direction != startPort.direction && // Проверяем направление
                   endPort.node != startPort.node; // Проверяем, что это не один и тот же узел
        }).ToList();
    }
